                            ...
                            <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                                ...
                                <tbody>
                                    @foreach ($alldue as $key => $item)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td><img src="{{ asset($item->customer->image) }}" alt=""
                                                    style="width: 50px; height: 50px" class="rounded-circle"></td>
                                            <td>{{ $item['customer']['name'] }}</td>
                                            <td>{{ $item->order_date }}</td>
                                            <td>{{ $item->payment_status }}</td>
                                            <td> <span class="btn btn-info waves-effect waves-light">
                                                {{ $item->total }} </span> </td>
                                            <td> <span class="btn btn-warning waves-effect waves-light">
                                                    {{ round($item->pay) }} </span> </td>
                                            <td><span class="btn btn-danger waves-effect waves-light">
                                            ...
                                            <td>
                                                ...
                                                <button type="button" class="btn btn-primary waves-effect waves-light rounded-pill"
                                                    data-bs-toggle="modal" data-bs-target="#signup-modal" onclick ="orderDue(this.id)" id ="{{ '/'.$item->id }}">Pay Due</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
...
<script type="text/javascript">
    ...
    function orderDue(id){
        //this ajax capture the specific id data
        $.ajax({
            type: 'GET',
            url: '/order/due'+id,
            dataType: 'json',
